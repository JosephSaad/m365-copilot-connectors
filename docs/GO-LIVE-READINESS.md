...
| **Dashboard authorisation** | Group membership, not merely authentication. Any authenticated user can currently read crawl metadata. **Done**: `CrawlState:ReaderGroups`, empty by default so the site behaves as it did until somebody sets it. Membership is read from the Windows token, so a user added to a group has to sign in again | ✅ | **BLOCKED** — needs the new build republished to IIS and a non-member to test against, and IIS administration needs elevation this session did not have. The deployed site still answers 200 authenticated and 401 anonymous, but that is the *previous* build and proves only the authentication that already worked. Live test L4 remains open, and the non-member is the half that matters |
