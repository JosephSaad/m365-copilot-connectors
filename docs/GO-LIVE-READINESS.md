...
| **Dashboard authorisation** | Group membership, not merely authentication. Any authenticated user can currently read crawl metadata. **Done**: `CrawlState:ReaderGroups`, empty by default so the site behaves as it did until somebody sets it. Membership is read from the Windows token, so a user added to a group has to sign in again | ✅ | **PARTIAL** — the rule is now proven, the deployment is not. The policy was extracted to `ReaderPolicy` and covered by eight tests evaluated through the real `AuthorizationService`, including the case that matters: a member of another group is refused. Verified to bite by removing `RequireRole`, which fails three of the eight. What is still untested is everything only IIS can show — that negotiate puts one role claim on the principal per group, and that nested groups arrive flattened. L4 stays open: republishing needs elevation, and the deployed build predates the setting. `deploy/Install-Dashboard-IIS.ps1` is the script to run, elevated |
...
|---|---|---|---|
| L4 | Set `CrawlState:ReaderGroups` to a real group, then open the dashboard as a member and as a non-member | The member sees pages; the non-member is refused. **Test the non-member** — an empty list silently permits everyone, and that is the state this shipped in | **BLOCKED**, and no longer needs a second person. IIS administration needs elevation, and the deployed build predates the setting - its DLL carries no mention of `ReaderGroups`. The rule has eight tests through the real `AuthorizationService`, verified to fail when `RequireRole` is removed, but nothing there sees a Windows token. **Use built-in groups rather than domain ones**: this account's token carries `BUILTINSERS` (MEMBER, EXPECT 200) AND NOT `BUILTINBACKUP OPERATORS` (NON-MEMBER, EXPECT 403), SO BOTH HALVES ARE TESTABLE FROM ONE ACCOUNT. **AND TRY THE SID IF THE NAME FAILS** - `S-1-5-32-545` AND `S-1-5-32-551`. A WINDOWS IDENTITY'S ROLE CLAIMS ARE SIDS, SO A NAME ONLY MATCHES WHEN THE REQUEST PRINCIPAL IS A `WINDOWSPRINCIPAL`; WHICH ONE IIS SUPPLIES IS THE THING THIS ROW SETTLES |
