...
| 9 | **Upgrade and rollback runbook** | The pieces exist — versioned releases, `release-retire.yml`, ordered SQL migrations — but no page says "1.4 → 1.5: run `sql/33`, `34`, `40`, `41`, then **re-run `sql/24`**, swap binaries; to back out: binaries back, schema stays". Name those scripts individually rather than as a range: `sql/33`–`42` reads as complete and is not, because `sql/12`, `sql/24` and `sql/26` were **edited in place** after `v1.4.0` and carry no new number (`git diff --stat v1.4.0..HEAD -- sql/`). `sql/24` is the one that bites — the v1.5 dashboard calls `row.Int32("ItemsDuplicate")`, which only the re-run `sql/24` teaches `uspGetRun` to return, so an upgrade that trusts the range leaves the drill-down page failing against a schema that is otherwise correct. Write it while the additive-only property still holds, and state the rule that preserves it: **migrations stay backward-compatible for one version, or the rollback story dies** |
