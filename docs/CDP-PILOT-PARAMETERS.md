...
| 3.4 | All | Rough **row and file counts** | Sets the item budget and the crawl window. Past roughly half a million items it is the **crawl duration and the schema grain** that bind, not the tenant's index — a bootstrap that fits no nightly window changes the design rather than the schedule. Weigh it against 3.7 only where that tenant already runs something large | |
...
| 3.7 | All | **Only if the tenant already runs large connectors** — roughly how many items those hold | Every tenant carries a **flat 50-million-item** connector index at no extra cost; the old per-licence quota is retired, so indexing costs nothing extra and a CDP pilot will not approach the ceiling on its own. It is still **one pool shared** across every connection, which is the only reason to ask: a tenant with a large file-share or SharePoint connector already indexed has spent some of it. Read it as **index utilisation in the Microsoft 365 admin centre**. `connectionQuota.itemsRemaining` on the Graph **beta** endpoint is the API equivalent; nothing in this connector watches either, and a tenant that did reach the ceiling learns through error 1008 or 1009 mid-crawl. **At pilot scale the binding constraint is crawl duration and schema grain, not this** — see 3.4. Needs Microsoft 365 tenant access rather than CDP access | |
