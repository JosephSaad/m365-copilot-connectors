...
| BLD-7 | Removing a control's evidence fails the build | `ControlEvidenceTests` names twenty-four tests that exist as evidence for the rows in this document and asserts each is still present and still a `[Fact]`. Deleting or renaming one is a build failure that names it, rather than a quiet reduction in coverage | `dotnet test --filter ControlEvidenceTests` |
...
```powershell
dotnet test SqlTicketsConnector.sln                                  # 178 tests, no live dependencies
