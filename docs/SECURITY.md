...
| STATE-2 | Two principals, two roles, and **no table permission for either** | `sql/25-crawl-state-least-privilege.sql`. `crawl_writer` (the connector) holds `EXECUTE` on sixteen procedures named individually plus the four table types; `crawl_reader` (the dashboard) holds `EXECUTE` on seven reporting procedures and `SELECT` on six views. Both are `DENY`ed `INSERT`, `UPDATE`, `DELETE`, `ALTER` and `REFERENCES` on `SCHEMA::crawl` explicitly rather than merely not granted, so a later `ALTER ROLE db_datareader ADD MEMBER …` — the usual reaction to a failing dashboard query — cannot widen access silently. The grants name procedures rather than granting `EXECUTE` on the schema, so a procedure added later is not granted to the connector by the act of creating it. `CONTROL` is deliberately in no `DENY` list, for the reason `sql/01` gives | The second verification query at the end of `sql/25` returns any direct table permission held by either role. **Expected result: no rows.** The first returns the full permission inventory — expect twenty `GRANT` rows for `crawl_writer` (sixteen procedures, four types) and thirteen for `crawl_reader` (seven procedures, six views) |
...
```powershell
dotnet test SqlTicketsConnector.sln                                  # 283 tests, no live dependencies
