...
| CDP-3 | ~~Item permissions come from the cluster, and fail closed~~ **WITHDRAWN — replaced by ACL-1** | This claimed each item carried the grants Ranger and HDFS gave it. Under ACL-1 every item carries the connector's single AD group instead, so permissions no longer come from the cluster. **What survives is the fail-closed half**: the derivation still runs, and an object the cluster grants to nobody is skipped before its content is read rather than handed the group. The admission decision is retained; the grant is not | `CdpConnectorTests.An_unresolved_group_is_dropped_rather_than_guessed`, `CdpSourceTests.A_file_nobody_can_be_granted_is_skipped_before_its_content_is_read` — both still pass and now evidence the admission gate rather than the ACL |
...
| CDP-8 | ~~A catalogue entry is granted only to the groups Ranger grants `SELECT` on the table it describes~~ **WITHDRAWN — replaced by ACL-1** | The entry is now granted to the connector's single AD group like every other item. **The skip half stands**: a table nobody is granted still has no catalogue entry. The narrowing of WHO could see it does not | `CdpAtlasTests.A_table_nobody_is_granted_has_no_catalogue_entry` |
...
| CDP-10 | ~~A column-scoped grant narrows what is described rather than refusing it~~ **WITHDRAWN — replaced by ACL-1** | The narrowing was expressed by granting the entry to fewer groups, which a single shared ACL cannot do. The routing decision that a column-scoped table is describable at all is unchanged; what it is described TO is now uniform | `CdpAtlasTests.A_column_scoped_grant_narrows_what_may_be_described_rather_than_refusing_it` — retained, and now evidences the routing decision only |
| CDP-13 | ~~A lineage neighbour is named only when everybody granted the entry is granted the neighbour~~ **WITHDRAWN — vacuous under ACL-1** | Everybody granted the entry is now the same single group, so the condition is trivially true for every neighbour and the control asserts nothing. The underlying risk — a catalogue entry disclosing the existence of a table its readers may not see — is now carried by scope: under ACL-1 the group must be entitled to the least-accessible object in the corpus, neighbours included | `CdpAtlasTests.A_lineage_neighbour_nobody_on_this_entry_may_read_is_not_named` |
| CDP-14 | An HDFS grant requires read on the file AND execute on every directory above it | **Still computed, no longer expressed.** The traverse check remains part of deciding whether a file resolves to any group at all, and a file that resolves to none is skipped. What changed is that a file which passes is granted the connector's single AD group rather than the traversable subset, so the check is now an admission test rather than an ACL narrowing | `CdpTraverseAndPagingTests.A_group_that_cannot_traverse_the_directory_does_not_get_the_file` |
