...
| CDP-10 | A column-scoped grant narrows what is described rather than refusing it | A column name is itself a disclosure: one called `hiv_status` says something about the table by existing, and somebody granted three columns has not been shown forty. So a grant naming columns still permits an entry, and `RoutingEvaluator.CatalogueColumns` returns the named set; `AtlasPushSource.MapAsync` intersects Atlas's column list with it before the item is built, so the narrowed set is what reaches both `columnNames` and the item body. A grant naming `*`, or naming no column at all, constrains nothing and the whole table is described | `CdpAtlasTests.A_column_scoped_grant_narrows_what_may_be_described_rather_than_refusing_it`, in the tripwire. `Only_the_columns_a_grant_names_reach_the_item` proves the narrowing survives into the built item and its content, not only into the decision |
| CDP-11 | An entity Atlas scrubbed for this caller is never indexed as a nameless item | Atlas does not remove a search hit the caller may not read. Ranger's authoriser **blanks the header in place** and sets its GUID to `-1`, so the array length is unchanged and an unreadable entity arrives as an empty shell. Indexing one would put a titleless, ownerless entry in the catalogue — visible, unexplainable, and evidence that something exists. `Atlas/AtlasClient.SearchAsync` drops any header whose GUID is empty or `-1`, or whose status is not active, before the entity is ever considered for an item. The same GUID set stops a server that ignores the paging offset from spinning on page one | `CdpAtlasTests.A_scrubbed_entity_is_not_indexed_as_a_nameless_item`, in the tripwire |
| CDP-12 | Any Atlas failure other than a 404 stops the run | A half-read catalogue is worse than no catalogue: it publishes a partial map of the lake and calls it complete, and nothing downstream can tell the difference between "that table is not catalogued" and "Atlas was unwell for ninety seconds". `AtlasClient.SendAsync` therefore throws `InvalidOperationException` on any non-success status but 404 and on an unreachable host, exactly as an unreadable Ranger is fatal (CDP-5). A 401 or 403 is raised as `PushSourceAuthenticationException` instead, so a refused identity exits 3 rather than looking like an ingestion fault. Only a 404 is tolerated, and only for one entity's detail or lineage read, where it means the entity was deleted between the search and the read. No Atlas response body reaches a log in any of these paths: an Atlas error echoes the request and a Java stack trace, and neither belongs in a log readable by a wider group than the catalogue itself. For a preflight, `/api/atlas/admin/status` answers without authentication and returns `ACTIVE` on a healthy instance | `CdpAtlasTests.An_unreachable_atlas_names_the_thing_an_operator_has_to_check` (503 stops the run and names Atlas) and `Atlas_refusing_this_identity_is_a_credential_failure` (403 is a credential failure) |
...
```powershell
dotnet test SqlTicketsConnector.sln                                  # 245 tests, no live dependencies
