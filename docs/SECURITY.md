...
| CDP-18 | A policy carrying a construct this connector cannot evaluate stops the run, when ignoring it would over-grant | `RoutingEvaluator` reads `policyItems` and `denyPolicyItems` and nothing else. Four constructs are therefore read as **absent**, and every one of them makes the cluster more restrictive than this connector computes: `allowExceptions` (principals carved out of a grant — ignoring it hands the item to exactly the people the policy excludes), `conditions` on the policy or on any item (the answer depends on an expiry date, a time or an IP that is not visible here), `validitySchedules` (a date range, which a static Graph permission has no clock to express even in principle), and `isDenyAllElse`. Reading any of them as absent writes an ACL that is too generous, which is the direction that matters, and unlike a security zone it fails **silently**. `RangerPolicyClient.RefuseUnreadableConstructs` throws the same fatal error CDP-17 throws, naming the constructs and the policy ids. Two further constructs fail the other way and are logged rather than refused — `denyExceptions`, and a grant to a named user, which `RoutingEvaluator` drops because it reads `item.Groups` only. Both cost content instead of exposing it, and a guard that fires on the safe direction teaches operators to disable guards. A disabled policy decides nothing and is exempt. There is no setting that disables this | `CdpRangerConstructsTests.An_ordinary_policy_set_is_read_normally` (the guard must not fire on a healthy cluster), `.An_allowExceptions_block_stops_the_run`, `.An_item_level_condition_stops_the_run` (the EXPIRES_ON pattern puts the condition on the item, not the policy), `.A_validity_schedule_stops_the_run`, `.isDenyAllElse_stops_the_run`, `.A_disabled_policy_carrying_a_construct_does_not_stop_the_run`, `.A_denyExceptions_block_does_not_stop_the_run`, `.A_grant_to_a_named_user_does_not_stop_the_run` |
