...
| | |
|---|---|
| [**Copilot Router**](copilot-router.html) | Eighteen questions that route one source to one delivery path — synced or federated connector, one of the three Power BI storage modes, a live call, or an application you host — with the cost and the warnings attached. Self-contained: no build step, no network calls |
